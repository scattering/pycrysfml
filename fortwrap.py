...
cpp_type_map = {'INTEGER':{'':'int*','1':'signed char*','2':'short*','4':'int*','8':'long long*','C_INT':'int*'}, 
                'REAL':{'FFTKIND':'long double*','fftkind':'long double*','DP': 'double*', 'dp': 'double*','SP': 'float*', 'sp': 'float*', 'CP': 'float*', 'cp': 'float*', '':'float*', '4':'float*', '8':'double*', 'C_DOUBLE':'double*', 'C_FLOAT':'float*'},
                'LOGICAL':{'':'int*'}, 
                'CHARACTER':{'':'char*'}, 
                'INT':{'':'int'}, 'COMPLEX':{'':'complex*', 'fftkind':'complex*', 'SP':'complex*','CP':'complex*',  'FFTKIND':'complex*', 'dp':'complex*','DP':'complex*',  'CP':'complex*', 'cp':'complex*', '8':'complex*','4':'complex*'}}
...
def write_constructor(file,object,fort_ctor=None):
    ...
    file.write('  ' + mangle_name(fort_wrap_file, 'allocate1_' + object.name) + '(&data_ptr); // Allocate Fortran derived type\n')
...
def write_destructor(file,object):
    ...
    file.write('  ' + mangle_name(fort_wrap_file,'deallocate1_'+object.name) + '(data_ptr); // Deallocate Fortran derived type\n')
    file.write('}\n\n')    
...
def write_class(object):
    ...
    if object.name!=orphan_classname:
        file.write('  void ' + mangle_name(fort_wrap_file, 'allocate1_'+object.name) + '(ADDRESS *caddr);\n')
        file.write('  void ' + mangle_name(fort_wrap_file, 'deallocate1_'+object.name) + '(ADDRESS caddr);\n')
    for proc in object.procs:
